...
def to_text(soup):
  ret = ''.join([e for e in soup.recursiveChildGenerator() if isinstance(e,unicode)]).replace('&nbsp;', ' ')
  ...
  def get(self, user):
    ...
    if user:
      ...
      for page_fav in page_favs:
        author = r1.sub(lambda x: x.group(1), page_fav.find('p', { 'class' : 'favorited_user noteTxt' }).find('a')['href'])
        #name = to_text(page_fav.find('p', { 'class' : 'favorited_user noteTxt' }).find('a'))
        template_values['favs'].append({
          'title' : to_text(page_fav.find('p', { 'class' : 'message description' })).strip(),
